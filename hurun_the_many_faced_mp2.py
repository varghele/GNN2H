...
for itm in range(len(DB_order)):
    graph_p1 = latent_graph_data[latent_graph_dict[DB_order["Molecule1"].iloc[itm]]]
    graph_p2 = latent_graph_data[latent_graph_dict[DB_order["Molecule2"].iloc[itm]]]
    graph_pm = latent_graph_data[latent_graph_dict[DB_order["Membrane"].iloc[itm]]]
...
class EdgeModel(torch.nn.Module):
    def __init__(self):
        ...
        in_channels = NO_EDGE_FEATURES+2*NO_NODE_FEATURES
        hidden=64
...
class NodeModel(torch.nn.Module):
    def __init__(self):
        ...
        hidden=32
        in_channels_2 = hidden+NO_NODE_FEATURES
...
class GlobalModel(torch.nn.Module):
    def __init__(self):
        ...
        hidden = 64
        in_channels_1=NO_NODE_FEATURES+NO_EDGE_FEATURES
        in_channels_2=hidden+NO_NODE_FEATURES

        self.global_mlp_1 = Seq(Lin(in_channels_1, hidden), LeakyReLU(), Lin(hidden, hidden))
        self.global_mlp_2 = Seq(Lin(in_channels_2, hidden), LeakyReLU(), Lin(hidden, NO_GRAPH_FEATURES))
