...
class GNN_FULL_CLASS(torch.nn.Module):
    ...
    def forward(self,plist):
        ...
        for _ in range(self.no_mp_one):
            enc_x_p1, _, u1 = self.meta1(x=enc_x_p1, edge_index=ei_p1, edge_attr=enc_ea_p1, u=u1, batch=btc_p1)
            enc_x_p2, _, u2 = self.meta1(x=enc_x_p2, edge_index=ei_p2, edge_attr=enc_ea_p2, u=u2, batch=btc_p2)
            enc_x_pm, _, um = self.meta1(x=enc_x_pm, edge_index=ei_pm, edge_attr=enc_ea_pm, u=um, batch=btc_pm)
        ...
        for _ in range(self.no_mp_two):
            nu_Batch.x, _, nu_Batch.y = self.meta3(x=nu_Batch.x, edge_index=nu_Batch.edge_index,
